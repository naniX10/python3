...
# def SungJukProgram(): # 함수에 너무많은 기능 부여
#     pass #하나의 기능만 부여
...
# 객체를 호출하면 클래스에 정의해둔 __str__가 호출되어 결과 출력
print(redCar)

# 성적 처리를 위한 성적SungJuk 클래스를 정의
...
sj1 = SungJuk('길동', 100, 50, 90)
print(sj1)

# 사원 관리를 위한 사원Employee 클래스를 정의
# 사원번호, 성, 이름, 이메일, 전화번호, 입사일, 직책, 급여, 수당, 상사번호,
# 부서번호로 구성
class Employee:
    ...
    def __str__(self):
        result = f' {self.empno} {self.lname} {self.fname} {self.email} ' \
                 f'{self.phone} {self.hdate} {self.sal} {self.comm} ' \
                 f'{self.mgr} {self.deptid}'
        return result


emp = Employee(1001, 'Michael', 'Jackson', 'MJ@gmail.com', '1231231230',
               '2002-01-05', 'AD_PRES', 50000, '', '')
